...
const quadData = new Float32Array([
    // First triangle
    -1.0, -1.0, 0.0,  0.0, 0.0,  // (x, y, z, u, v)
     1.0, -1.0, 0.0,  1.0, 0.0,  // (x, y, z, u, v)
     1.0,  1.0, 0.0,  1.0, 1.0,  // (x, y, z, u, v)

    // Second triangle
    -1.0, -1.0, 0.0,  0.0, 0.0,  // (x, y, z, u, v)
     1.0,  1.0, 0.0,  1.0, 1.0,  // (x, y, z, u, v)
    -1.0,  1.0, 0.0,  0.0, 1.0   // (x, y, z, u, v)
]);
...
class TriggerQuad
{
    ...
    {
        this.gl = gl;
        this.mesh = new Mesh(gl, "quad");
        this.mesh.Initialize(quadData);

        //8 metaballs = (float3 + float3) * 8
        // let arraySize = 8 * 6;
        // this.metaBallData = new Float32Array(arraySize);
        // this.metaBallBuffer = gl.createBuffer();
    }

    // FillMetaBallBuffer()
    // {
    //     this.gl.bindBuffer(this.gl.ARRAY_BUFFER, this.metaBallBuffer);
    //     this.gl.bufferData(this.gl.ARRAY_BUFFER, this.metaBallData, this.gl.STATIC_DRAW);
    // }

    // AddMetaBall(index, position, colour)
    // {
    //     let arrayIndex = index * 8 * 6;
    //     this.metaBallData[arrayIndex + 0] = position.x;
    //     this.metaBallData[arrayIndex + 1] = position.y;
    //     this.metaBallData[arrayIndex + 2] = position.z;

    //     this.metaBallData[arrayIndex + 3] = colour.x;
    //     this.metaBallData[arrayIndex + 4] = colour.y;
    //     this.metaBallData[arrayIndex + 5] = colour.z;
    // }

    Draw(u_FragColor, a_Position, program) {
        // Bind the buffer before setting vertex attributes and drawing
        this.gl.bindBuffer(this.gl.ARRAY_BUFFER, this.mesh.vertexBuffer);
        ...
        var sphereRadius = this.gl.getUniformLocation(program, "sphereRadius");
        this.gl.uniform1f(sphereRadius, 1.5);
...
class MetaBall {
    constructor() {
        this.position = new MyVector3(0, 0, 1);
        this.direction = new MyVector3(-1 + 2 * Math.random(), -1 + 2 * Math.random(), -1 + 2 * Math.random());
